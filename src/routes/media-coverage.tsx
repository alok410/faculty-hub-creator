...
export const Route = createFileRoute("/media-coverage")({
  validateSearch: (search: Record<string, unknown>): MediaSearch => {
    ...
    const validTabs: Platform[] = ["youtube", "linkedin", "facebook", "instagram", "twitter"];
    return {
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "GTU-ITR social media presence — YouTube, LinkedIn, Facebook, Instagram & Twitter." },
      { property: "og:title", content: "Media Coverage | GTU-ITR" },
      { property: "og:description", content: "GTU-ITR social media presence — YouTube, LinkedIn, Facebook, Instagram & Twitter." },
...
function Page() {
  ...
  const active: Platform = tab ?? "youtube";

  const platformMeta = PLATFORMS.find((p) => p.key === active)!;

  const posts = useMemo(
    () =>
      SOCIAL_POSTS.filter((p) => p.platform === active).sort(
  ...
  return (
    <PageShell title="Media Coverage" subtitle="Follow GTU-ITR across social media — explore our latest posts, videos and updates.">
      ...
      <div className="mb-6 flex items-center gap-3">
        <span style={{ color: platformMeta.color }}>{platformMeta.icon}</span>
        <h2 className="section-title text-xl md:text-2xl" style={{ color: platformMeta.color }}>
          {platformMeta.label} Posts
        </h2>
      </div>

      {/* ── Post grid ───────────────────────────────────────── */}
      {posts.length === 0 ? (
        <p className="py-12 text-center text-muted-foreground">No posts available for this platform yet.</p>
      ) : (
        <Stagger key={active} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <StaggerItem key={post.id} className="group h-full">
              <a
                href={post.postUrl}
                target="_blank"
                rel="noopener noreferrer"
                id={`post-${post.id}`}
                className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl"
                style={{ borderTopColor: platformMeta.color, borderTopWidth: "3px" }}
              >
                {/* Thumbnail */}
                <div className="relative h-48 overflow-hidden bg-muted">
                  ...
                  {/* Platform badge */}
                  <div
                    className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-white shadow-lg backdrop-blur-sm"
                    style={{ backgroundColor: `${platformMeta.color}dd` }}
                  >
                    {platformMeta.icon}
                ...
                <div className="flex flex-1 flex-col justify-between p-5">
                  <div>
                    <p className="line-clamp-2 text-[15px] font-medium leading-snug text-foreground transition-colors duration-200 group-hover:text-brand-navy">
                      {post.title}
                    </p>
                  </div>
